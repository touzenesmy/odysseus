...
Composer toolbar toggles: `static/index.html` exposes per-mode icon toggles (same `input-icon-btn` style as the web-search button) in the order web search, bash, **Documents**, then the workspace name. The bash and Documents buttons are agent-mode-only (hidden in chat mode) and are covered by the mobile overflow collapse. `static/app.js` `setupToggle()` wires each to a hidden checkbox with localStorage persistence, `aria-pressed`, a splash-card entry, and a toast; `static/js/chat.js` appends the checkbox state to the chat form as `allow_bash` / `allow_documents` (`true` when ticked, `false` when the toggle is visible but unticked, omitted in chat mode so the route falls back to privilege defaults). A ticked Documents toggle force-includes the editor-panel document tools (`create_document`, `edit_document`, `update_document`, `suggest_document`, `manage_documents`) for that turn, so `edit_document` — the patch-the-open-document path preferred over bash edits — survives tool-retrieval variance.
