...
Light auto-escalation strip: when a chat-mode message is escalated to agent by
the conservative regexes (web/notes/email/etc. intent) but not by a
workspace/shell intent, the route removes `bash`, `python`, `read_file`,
...
Attachment-only sends are valid. Missing or unauthorized ids are skipped during preprocessing, while a missing/wrong-owner durable reference aborts a message/history replacement before existing transcript rows are removed. Upload failures keep pending files for retry, unsupported media can degrade to text markers, optional Office/PDF/VL dependencies can emit extraction banners, Office attachments can create markdown documents when extracted server-side, and fillable-PDF auto-document failures fall back to normal PDF extraction. `chat_messages.content` and FTS do not retain provider data URLs; structured references stay in metadata for reloads. Chat does not own upload bytes or durable document storage; it requests document/upload behavior from those subsystems. Upload rejection display (2026-09-02, `6546a007`, upstream #6236): on an HTTP failure `static/js/fileHandler.js` stashes the server reason in module state (`_lastUploadError`, exposed via `getLastUploadError()`, cleared on success, with a dict-`detail` guard for the `FILE_TOO_LARGE` path) and no longer toasts it itself; `chat.js` shows it ONCE in the composer toast ("Upload failed: <reason>. Attachment kept so you can retry.") — before the fix the HTTP path toasted the specific reason and the send path also toasted a generic message, so one failure produced two toasts.

Frontend chat distinguishes normal resend from regenerate-from-here: normal resend appends a fresh user copy and carries upload IDs where available, while regeneration truncates from the selected point. Message delete prompts before removing the WHOLE turn: the owning user bubble plus every assistant row of that turn (a turn spans multiple assistant rows — tool-approval question row + response row, interrupted/continued runs), from the user bubble to the next user bubble, collecting every distinct `dataset.dbId` in the range (2026-09-15: pairing only the first AI bubble orphaned the rest — surviving rows with no user prompt corrupted context after refresh). An AI bubble with no preceding user bubble (its prompt already deleted) deletes its own bubble-to-next-user range. Desktop Enter submits; mobile Enter inserts a newline unless another platform-specific send control is used.
